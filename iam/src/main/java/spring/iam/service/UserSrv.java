...
public class UserSrv implements IUserSrv {
  ...
  public void ensureNotExistedByUsername(String username) {
    if (userRepo.countExistedByUsername(username) > 0) throw new ServiceExc(Failed.ALREADY_EXISTED);
  }
  ...
  public Set<Role> ensureOwningExistedByIds(Set<Long> ids) {
    ...
    if (CollectionUtils.isEmpty(owning)) throw new ServiceExc(Failed.OWNING_SIDE_NOT_EXISTS);
  ...
  public UserRes findByUsername(String username) {
    try {
      ...
      if (!queried.isPresent()) throw new ServiceExc(Failed.NOT_EXISTS);
      return userMpr.asResponse(queried.get());
    } catch (ServiceExc e) {
      throw e;
    } catch (Exception e) {
