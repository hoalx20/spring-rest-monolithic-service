...
public class DaoAuthenticationFlt extends OncePerRequestFilter {
	...
	protected void doFilterInternal(@NonNull HttpServletRequest request, @NonNull HttpServletResponse response, @NonNull FilterChain filterChain)
		...
		try {
			...
			immediatelyTerminate(response, causeBy.getHttpStatus().value(), causeBy.getCode(), causeBy.getMessage());
		} catch (Exception e) {
			e.printStackTrace();
	...
	private void immediatelyTerminate(HttpServletResponse response, int httpStatus, int code, String message) throws IOException {
	...
	private HashMap<String, Object> buildDetails(SignedJWT signedJWT) throws Exception {
		...
		details.put("user", new AuthenticationDetail(username, userId, sessionId, sessionExpiredAt, scope, referId));
		return details;
	}
}
